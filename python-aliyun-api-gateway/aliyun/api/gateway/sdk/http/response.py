...
class Response(Request):
    ...
    def get_http_response(self):
        if self.__port is None or self.__port == "":
            self.__port = 80
        try:
            self.__connection = client.HTTPConnection(self.parse_host(), self.__port)
    ...
    def get_http_response_object(self):
        if self.__port is None or self.__port == "":
            self.__port = 80
        try:
            self.__connection = client.HTTPConnection(self.parse_host(self.get_host()), self.__port)
            self.__connection.connect()
    ...
    def get_https_response(self):
        try:
            self.__port = 443
            self.__connection = client.HTTPSConnection(self.parse_host(), self.__port,
    ...
    def get_https_response_object(self):
        ...
        try:
            self.__port = 443
            self.__connection = client.HTTPSConnection(self.get_host(), self.__port, cert_file=self.__cert_file,
                                                        key_file=self.__key_file)
            self.__connection.connect()
            self.__connection.request(method=self.get_method(), url=self.get_url(), body=self.get_body(),
                                      headers=self.get_headers())
